...
from API.models import StoreProduct, StoreCategories

from API.models import FactoryProduct, FactoryCategories
...
def build_it():
    ...
    while str(time_zero) != '2022-04-04 00:00:00':
        for i in range(3):
            stat = StatStore(product_count=randrange(1000), product=StoreProduct.objects.get(id=randrange(4) + 1),
                             category=StoreCategories.objects.get(id=randrange(4) + 1), date=time_zero)
            stat.save()
        time_zero += datetime.timedelta(hours=1)
    ...
    while str(time_zero) != '2022-04-04 00:00:00':
        for i in range(3):
            stat = StatFactory(count=randrange(1000), product=FactoryProduct.objects.get(id=randrange(4) + 1), machine=Machine.objects.get(id=randrange(3) + 1),
                               category=FactoryCategories.objects.get(id=randrange(4) + 1), date=time_zero, serial_num=serial)
            stat.save()
        serial += 1
        time_zero += datetime.timedelta(hours=1)